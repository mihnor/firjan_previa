#!/usr/bin/env python3
"""
FIRJAN XR - GitHub Update & Video Generation Script
==================================================
Automatiza:
1. Atualização dos letterings numerados no GitHub
2. Preparação para geração de vídeos com narrações
3. Organização da estrutura de assets
"""
...
class FirjanGitHubUpdater:
    def __init__(self, project_root):
        ...
        # Mapeamento de letterings para boards
    ...
    def organize_letterings_for_github(self):
        ...
        for lettering_file, board_folder in self.lettering_mapping.items():
            ...
            if source_path.exists():
                ...
                print(f"  ✅ {lettering_file} → {board_folder}")
            else:
    ...
    def prepare_video_structure(self):
        ...
        for board_folder in set(self.lettering_mapping.values()):
            ...
            if board_path.exists():
                for lettering in board_path.glob("LETTERING_*.png"):
                    video_config["letterings"].append(str(lettering))
            
            # Buscar áudio correspondente (se existir)
            audio_pattern = f"*{board_folder.split('_')[-1].lower()}*"
            audio_files = list(self.audio_source.glob("*.mp3")) + list(self.project_root.glob("*.wav"))
            
            for audio in audio_files:
                if any(keyword in audio.name.lower() for keyword in ["firjan", "tour", "guta"]):
                    video_config["audio"] = str(audio)
                    break
    ...
    def generate_ffmpeg_commands(self, video_configs):
        ...
        for config in video_configs:
            ...
            # Comando básico para imagem estática com áudio
            if config["audio"] and len(config["letterings"]) == 1:
                cmd = [
                    "ffmpeg", "-y",
                    "-loop", "1",
                    "-i", config["letterings"][0],
                    "-i", config["audio"],
                    "-c:v", "libx264",
                    "-t", "10",  # 10 segundos por padrão
                    "-pix_fmt", "yuv420p",
                    "-c:a", "aac",
                    str(output_path)
                ]
                commands.append({
                    "board": board_name,
                    "command": cmd,
                    "description": f"Vídeo simples: imagem + áudio"
                })
            
            # Comando para múltiplas imagens (slideshow)
            elif len(config["letterings"]) > 1:
                ...
                if config["audio"]:
                    cmd.extend(["-i", config["audio"], "-c:a", "aac"])
                
                commands.append({
                    "board": board_name,
                    "command": cmd,
                    "description": f"Slideshow: {len(config['letterings'])} imagens"
                })
    ...
    def create_github_commit_script(self, organized_files):
        ...
        with open(commit_script, 'w') as f:
            ...
            f.write("\n# Commit changes\n")
            f.write(f"git commit -m 'Update letterings numerados - {len(organized_files)} files'\n\n")
    ...
    def generate_summary_report(self, organized_files, video_configs, ffmpeg_commands):
        ...
        report = {
            ...
            "total_size_mb": sum(f["size_mb"] for f in organized_files),
    ...
    def run_update(self):
        ...
        print("🚀 FIRJAN XR - Iniciando atualização...")
        print("=" * 50)
        ...
        print(f"📁 Arquivos organizados: {len(organized_files)}")
        print(f"🎬 Vídeos para gerar: {len(ffmpeg_commands)}")
        print(f"📊 Tamanho total: {report['total_size_mb']:.1f} MB")
        print("\n🔧 Próximos passos:")
        print(f"1. Execute: {commit_script}")
        print("2. Gere os vídeos com os comandos FFmpeg")
        print("3. Verifique o relatório gerado")
